...
export default function WordHoverTip() {
  ...
  const [enabled, setEnabled] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('eii_wordtip_disabled') !== '1';
    }
    return true;
  });

  // 分词: enabled 时监听 DOM 变化并防抖
  useEffect(() => {
    if (!enabled) return;
  ...
  const fmt = (s: WordSense): string => {
    ...
    else p.push(...Object.entries(s).filter(([k]) => !['pos', 'definition'].includes(k)).map(([, v]) => String(v)));
    return p.join(' ');
  };
    ...
    <>
      ...
      {state.visible && (
        ...
        >
          ...
          {state.def && !state.loading && Array.isArray(state.def.senses) && state.def.senses.length > 0 && (
...
function useAutoFlip(x: number, y: number, _visible: boolean) {
  const flip = typeof window !== 'undefined' ? y + 160 > window.innerHeight - 12 : false;
