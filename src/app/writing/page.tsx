...
import writing2023 from '../../../content/writing/2023.json';
...
export default function WritingPage() {
  ...
  const runReview = async () => {
    ...
    setLoading(true);
    try {
      const chartContext = data.keyPoints.join('\n');
  ...
  return (
    <section className="grid grid-cols-12 gap-3">
      {/* 左栏:图表与采分数据 */}
      <div
        className="col-span-12 lg:col-span-5 bg-white border border-zinc-200 rounded p-3 flex flex-col justify-between lg:h-[calc(100vh-68px)]"
        data-source="2023 写作"
      >
        <div>
          ...
          <div className="w-full h-56 border border-zinc-100 rounded bg-zinc-50/50">
            <ReactECharts option={data.chartOption} style={{ height: '100%', width: '100%' }} />
          </div>
        ...
        <div className="text-zinc-400 text-[11px] mt-2">
          提示:试着选中上方文字中的英文单词测试划词捕获。
        </div>
      </div>

      {/* 右栏:语料积木 + 编辑器 + AI批改 */}
      <div className="col-span-12 lg:col-span-7 bg-white border border-zinc-200 rounded p-3 flex flex-col justify-between lg:h-[calc(100vh-68px)]">
        <div>
          <div className="border-b border-zinc-100 pb-2 mb-2">
            ...
            {SCAFFOLD_GROUPS.map((g) => (
              <div key={g.key} className="flex flex-wrap gap-1.5 mt-2 items-center">
                <span className="text-[10px] text-zinc-400 font-mono w-16">{g.label}</span>
                {(data.scaffolding as Record<string, string[]>)[g.key].map((phrase) => (
                  <button
                    key={phrase}
