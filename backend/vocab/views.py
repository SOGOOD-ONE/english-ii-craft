    ...
    def post(self, request):
        ...
        if cached:
            return Response({
                ...
                'senses': cached.senses, 'collocations': cached.collocations,
                'from_cache': True,
            })
        # 调 AI
        ...
        return Response({
            ...
            'senses': obj.senses, 'collocations': obj.collocations,
            'from_cache': False,
        })
...
class CardViewSet(mixins.ListModelMixin,
    ...
    def get_queryset(self):
        qs = VocabCard.objects.filter(user=self.request.user).select_related('word')
    ...
    def create(self, request, *args, **kwargs):
        ...
        # 2. 找已存在卡(按 user + word + source_path 组合唯一)
        card, created = VocabCard.objects.get_or_create(
            user=request.user, word=word, source_path=(d.get('source_path') or ''),
            defaults={
    ...
    def review(self, request, pk=None):
        ...
        req.is_valid(raise_exception=True)
        fields = schedule_card(card, req.validated_data['rating'])
